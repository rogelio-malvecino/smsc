<?php
	...
	if ($_GET["Start"]=='2')
		...
		//echo $mStartDate.$mEndDate;
			...
			include ("datasource.php");
			$mResult = $mysqli->query("Call sp_GeneralLedger_Select('".$_REQUEST['ControlNo']."','"
			...
			$display_string .= "<table width=100% border=1 cellspacing=0 cellpadding=1 bordercolor=#FFFFFF>
								...
								<tr>
									<td width=8% class=title1 align=center>No.</td>
									<td width=12% class=title1 align=center>Account No.</td>
									<td width=15% class=title1 align=center>Account Name</td>
									<td width=23% class=title1 align=center>SubAccount No.</td>
									<td width=20% class=title1 align=center>SubAccount Name</td>
			...
			if (mysqli_num_rows($mResult) > 0)
				{
					...
					while ($ado = $mResult->fetch_array(MYSQLI_BOTH))
						{ 
							...
							if (strlen($ado["Month1"])==1)
							{
							$mMonth1 = "0".$ado["Month1"];
							}
							else
							{
							$mMonth1 = $ado["Month1"];
							}
							if (strlen($ado["Day1"])==1)
							{
							$mDay1 = "0".$ado["Day1"];
							}
							else
							...
							if (strlen($ado["Month2"])==1)
							{
							$mMonth2 = "0".$ado["Month2"];
							}
							else
							{
							$mMonth2 = $ado["Month2"];
							}
							if (strlen($ado["Day2"])==1)
							{
							$mDay2 = "0".$ado["Day2"];
							}
							else
						...
						*/
						 $display_string .= "<tr bgcolor=#EBEBEB onMouseOver=this.style.backgroundColor='#FFFFFF' onMouseOut=this.style.backgroundColor=''>
													...
													<td width=12% align=left class=detail1>
														<a href='#' onClick=javascript:GeneralLedger2_CallAjaxPage('generalledger_search2.php','1','".$ado["AccountID_cd"]."','','".$ado['Journal_Tx']."','".$mMonth1."','".$mDay1."','".$ado['Year1']."','".$mMonth2."','".$mDay2."','".$ado['Year2']."','".trim($ado["SubsidiaryID_cd"])."')>".$ado["DocNo_tx"]."</a>                                                                   	
													</td>
													<td width=15% align=left class=detail1>"
														.$ado["Reference_tx"]."&nbsp;
													</td>";
			...
			$display_string .= "<tr bgcolor=#FFFFFF>
								...
								<tr bgcolor='#FFFFFF'>
									<td colspan=8 align=center>
										<input type='button' name='cmdPrint' class='detail1' value='Print' onClick=javascript:GeneralLedger_Print('".$_SESSION['S_UserID']."','"
																																	  .$_REQUEST['ControlNo']."','"
																																	  .$_REQUEST['SubsidiaryNo']."','"
																																	  .$_REQUEST['Journal']."','"
																																	  .$_REQUEST['Month1']."','"
