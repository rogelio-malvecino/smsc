<?php
			...
			function Header()
				{
					...
					$this->Cell(20,6,"TRN No",1,0,'C');
					$this->Cell(25,6,"Reference No",1,0,'C');
					$this->Cell(50,6,"Payee",1,0,'C');
					$this->Cell(100,6,"Particular",1,0,'C');
	...
	$mResult = $mysqli->query("Call rp_GeneralLedger_Select('".$_SESSION['S_UserID']."','"
															  .$_REQUEST['ControlNo']."','"
			...
			while ($ado = $mResult->fetch_array(MYSQLI_BOTH))
				{
					...
					$pdf->SetFont('Arial','',9);	
					$pdf->Cell(1,6,'',0,0,'C');
					$pdf->Cell(20,6,substr($ado["DocDate_dt"],5,2).'/'.substr($ado["DocDate_dt"],8,2).'/'.substr($ado["DocDate_dt"],2,2),1,0,'C');
					$pdf->Cell(20,6,$ado["Ref_tx"].$ado["DocNo_tx"],1,0,'L');
					$pdf->Cell(25,6,$ado["Reference_tx"],1,0,'L');
					$pdf->Cell(50,6,$mPayee,1,0,'L');
					$pdf->Cell(100,6,substr($ado["Particular_tx"],0,45),1,0,'L');
