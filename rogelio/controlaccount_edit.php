<?php
	...
	$mAccess2 = "1";//fp_Get_Record("Access_yn","tb_muserrights", "UserID_cd = ''".$_SESSION['UserID']."'' AND FuncID_cd = ''COAEDIT''");
			...
			if (mysqli_num_rows($mResult) > 0)
				{
					...
					$mAccountDesc = $ado['AccountDesc_tx'];
					$mGroupID = $ado['GroupID_cd'];
					$mBank = $ado['Bank_yn'];
					$mNormal = $ado['Debit_yn'];
					$mCR = $ado['CR_yn'];
					$mPB = $ado['PB_yn'];
					$mCS = $ado['CS_yn'];
					$mBS = $ado['BS_yn'];
					$mCD = $ado['CD_yn'];
					$mGJ = $ado['GJ_yn'];
					$mBalanceSheet = $ado['BalanceSheet_yn'];
					$mBalanceSheetType = $ado['BalanceSheetType_tx'];
					$mIncomeStatement = $ado['IncomeStatement_yn'];
	...
	if ($_REQUEST['Start'] == 2)
		{													
			...
			$mResult = $mysqli->query("Call sp_ControlAccount_Update('".$_SESSION['S_UserID']."','"
																	   .$mAccountID."','"
																	   .$mAccountDesc."','"
																	   .$mGroupID."','"
																	   .$mBank."','"
...
<html>
<head>
	<title>Update Control Account</title>
	<link href="../global/mystyle.css" rel="stylesheet" type="text/css" >
	<script language="JavaScript" src="../global/Functions.js"></script>

</head>
<body background="../images/background.JPG" onLoad="false; document.frmFinancial.txtAccountDesc.focus();"> 
<form name="frmFinancial" action="controlaccount_edit.php" method="post">
												...
												<table width="100%" class="title1">
													<tr>
														<td colspan="4" class="title1" align="center">
                                                        	<font size="+1">CONTROL ACCOUNT (UPDATE)</font>
                                                        </td>
													</tr>
													<tr>
														<td colspan="4" align="center" nowrap class="title1">
                                                          	<input type="button" name="cmdNew" class="detail1" value="  Create New Record "  <?php if ($mAccess1 == '') { ?>disabled<?php } ?> onClick="javascript:eControlAccount_Action(1);">
                                                            <input type="button" name="cmdSave" class="detail1" value="  [F8] - Save  "  <?php if ($mAccess2 == '' || $mOk == 1) { ?>disabled<?php } ?> onClick="javascript:eControlAccount_Action(2);" tabindex="13">
                                                            <input type="button" name="cmdSearch" class="detail1" value="  Search/List Record(s)  " onClick="javascript:eControlAccount_Action(3);">
														</td>
													</tr>											  
											  </table>
											</td>
