...
function FPDF($orientation='P',$unit='mm',$format='A4')
{
	...
	if(is_string($format))
	{
		...
		elseif($format=='custom')
			$format=array(595.28,200);
		elseif($format=='games')
			$format=array(306,792);
		elseif($format=='checke')
			$format=array(612,450);
		elseif($format=='lphil')
...
function fp_Beginning_TrialBalance($mControlNo, $mJournal, $mStartDate, $mEndDate, $mCurrency, $mType)
	{
		...
		if (mysqli_num_rows($mResult) > 0)
			{
				...
				if ($mCurrency=='1')
					...
					{
						$mBegDebit = $ado["DebitDollar"];
						$mBegCredit = $ado["CreditDollar"];
					}	
			}				
		mysqli_close($mysqli);			
