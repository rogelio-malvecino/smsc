			...
			if ($_POST['Rec'] > 0)
				{
					...
					while ($i <= $_REQUEST['Rec'])
						{ 										
							...
							$mResult = $mysqli->query("Call sp_CashReceipts_Delete('".$_SESSION['S_UserID']."','"
																				  .$mCRID."','"
																				  .$mDesc."')");
							mysqli_close($mysqli);
							$i = $i + 1;
	...
	if ($_POST["Start"]=='1' || $_POST["Start"]=='2')
		{
			...
			$mResult = $mysqli->query("Call sp_CashReceipts_Search('".$_POST['ControlNo']."','"
																  .$_POST['ReferenceNo']."','"
																  .$_POST['StartDate']."','"
																  .$_POST['EndDate']."','"
			...
			$display_string .= "<table width=100% border=1 cellspacing=0 cellpadding=1 bordercolor=#FFFFFF>
									<tr bgcolor=#FFFFFF>
									<td colspan='3' align='left'><input type='button' name='cmdDelete' class='detail1' value='Delete Record(s)' onClick='javascript:CashReceipts_Delete();' ";
			...
			$display_string .= "<table width=100% border=1 cellspacing=0 cellpadding=1 bordercolor=#FFFFFF class=display id=example>
								<thead><tr class=title>
									...
									<td align=left width=2% class=title1>
										<input type='checkbox' name='chkControl' onClick='javascript:CashReceipts_chkAll();'>
									</td>
									<td align=center width=10% class=title1>CR#</td>
									...
									<td align=center width=13% class=title1>CS Amount</td>
			...
			if (mysqli_num_rows($mResult) > 0)
				{
							...
							if ($iDate <> $ado["CRDate_dt"])
								{
									...
									/*$display_string .= "<tr>
															<td colspan=7 class=title1 align=center>"
																.date("F d, Y", mktime(0,0,0,substr($ado["CSDate_dt"],5,2),substr($ado["CSDate_dt"],8,2),substr($ado["CSDate_dt"],0,4))).
															"</td>
														</tr>";
									*/					
								}
					...
					/*		$display_string .= 		"<td width='10%' align=left class=detail1>
														<input name=hidID".$i." type=hidden value='".$ado["CSID_cd"]."'>";
*/
							$display_string .= 		"<td width='10%' align=left class=detail1>
														<img src='images/printer.jpg' align='bottom' onClick=javascript:PrintVoucher('".$ado["CRID_cd"]."','".$ado["Amount_no"]."');>
														<input name=hidID".$i." type=hidden value='".$ado["CRID_cd"]."'>";


														if ($ado["Post_yn"]=='0')
															{
																//$display_string .= "<a href=cashsales_edit.php?Start=1&ID=".$ado["CSID_cd"].">".$ado["CSID_cd"]."</a>";
																$display_string .= "<a href='#' onClick=javascript:CashReceipts_CallAjaxPage('cashreceipts_edit.php','1','".$ado["CRID_cd"]."')>".$ado["CRID_cd"]."</a>";
															}
														else
															{
																$display_string .= $ado["CRID_cd"];
															}
								
							$display_string .= 		"</td>
													<td width='15%' align=left class=detail1>".$ado["CRDate_dt"]."&nbsp;</td>
	...
	if ($_POST["Start"]=='3' || $_POST["Start"]=='4')
		{
			...
			$mResult = $mysqli->query("Call sp_Execute_Query(
									  ...
									  FROM tb_mcoahdr a
									  WHERE a.AccountID_cd =''".$_POST['AccountID']."'' AND CS_yn= 1')");
	...
	if ($_POST["Start"]=='5')
		{
			...
			//$mResult = $mysqli->query("Call sp_SubsidiaryRights_Select('".$_POST['AccountID']."','".substr($_SESSION['BranchID'],6,7)."')");
						...
						{ 										
							...
							$display_string .= "<tr bgcolor=#EBEBEB onMouseOver=this.style.backgroundColor='#FFFFFF' onMouseOut=this.style.backgroundColor=''>
													...
													<td width=12% align=right class=detail13>
														<input type=text id='hidDebit".$i."' maxlength='13' name='hidDebit".$i."'  size='10' value=".number_format($mDebit,2)." class=detail13 dir=rtl onKeyUp=javascript:eCashReceipts_ComputeTotal_(".$i."); tabindex=14>
													</td>
													<td width=12% align=right class=detail13>
														<input type=text id='hidCredit".$i."' maxlength='13'  name='hidCredit".$i."' size='10' value=".number_format($mCredit,2)." class=detail13 dir=rtl onKeyUp=javascript:eCashReceipts_ComputeTotal_(".$i."); tabindex=15>
													</td>
												
														<input type=hidden id='mhidDebit".$i."' name='mhidDebit".$i."'  value=".number_format($mDebit,2)."  >
													
													
														<input type=hidden id='mhidCredit".$i."'   name='mhidCredit".$i."'  value=".number_format($mCredit,2)." >
			...
			if (mysqli_num_rows($mResult) > 0)
				{
					...
					$display_string .= " onclick='javascript:aCashReceipts_SetSearchVoyageReference(this.innerHTML);' class=suggest_link>";
