			...
			if ($_POST['Rec'] > 0)
				{
					...
					while ($i <= $_REQUEST['Rec'])
						{ 										
							...
							$mResult = $mysqli->query("Call sp_CashSales_Delete('".$_SESSION['S_UserID']."','"
																				  .$mCSID."','"
																				  .$mDesc."')");
							mysqli_close($mysqli);
							$i = $i + 1;
	...
	if ($_POST["Start"]=='1' || $_POST["Start"]=='2')
		{
			...
			$mResult = $mysqli->query("Call sp_CashSales_Search('".$_POST['ControlNo']."','"
																  .$_POST['ReferenceNo']."','"
																  .$_POST['StartDate']."','"
																  .$_POST['EndDate']."','"
			...
			$display_string .= "<table width=100% border=1 cellspacing=0 cellpadding=1 bordercolor=#FFFFFF>
									<tr bgcolor=#FFFFFF>
									<td colspan='3' align='left'><input type='button' name='cmdDelete' class='detail1' value='Delete Record(s)' onClick='javascript:CashSales_Delete();' ";
			...
			$display_string .= "<table width=100% border=1 cellspacing=0 cellpadding=1 bordercolor=#FFFFFF class=display id=example>
								<thead><tr class=title>
									...
									<td align=left width=2% class=title1>
										<input type='checkbox' name='chkControl' onClick='javascript:CashSales_chkAll();'>
									</td>
									<td align=center width=10% class=title1>CS#</td>
...
*/
							$display_string .= 		"<td width='10%' align=left class=detail1>
														<img src='images/printer.jpg' align='bottom' onClick=javascript:PrintVoucher('".$ado["CSID_cd"]."','".$ado["Amount_no"]."');>
														<input name=hidID".$i." type=hidden value='".$ado["CSID_cd"]."'>";
														...
														if ($ado["Post_yn"]=='0')
															{
																...
																$display_string .= "<a href='#' onClick=javascript:CashSales_CallAjaxPage('cashsales_edit.php','1','".$ado["CSID_cd"]."')>".$ado["CSID_cd"]."</a>";
															}
														else
															{
																$display_string .= $ado["CSID_cd"];
															}
								
							$display_string .= 		"</td>
													<td width='15%' align=left class=detail1>".$ado["CSDate_dt"]."&nbsp;</td>
						...
						{ 										
							...
							$display_string .= "<tr bgcolor=#EBEBEB onMouseOver=this.style.backgroundColor='#FFFFFF' onMouseOut=this.style.backgroundColor=''>
													...
													<td width=12% align=right class=detail13>
														<input type=text id='hidDebit".$i."' maxlength='13' name='hidDebit".$i."'  size='10' value=".number_format($mDebit,2)." class=detail13 dir=rtl onKeyUp=javascript:eCashSales_ComputeTotal_(".$i."); tabindex=14>
													</td>
													<td width=12% align=right class=detail13>
														<input type=text id='hidCredit".$i."' maxlength='13'  name='hidCredit".$i."' size='10' value=".number_format($mCredit,2)." class=detail13 dir=rtl onKeyUp=javascript:eCashSales_ComputeTotal_(".$i."); tabindex=15>
													</td>
